...
export function Navbar({ user }: NavbarProps) {
  return (
    // A phone on its side under 768px wide isn't affected by `md:grid` and falls back to
    // the stacked column, which would take up an extra line of space in a
    // vertically short layout. To keep the navbar on one line, justify-between
    // distributes a hidden <div /> spacer on the left, the title centrally and the
    // logout button to the right. The query carries its own `max-width` because
    // the short-landscape one appears after `md:` (in wider landscape phones)
    // Tablets need nothing: they clear 768px in landscape already.
    <div className="col-span-full flex flex-col items-center gap-2 desktop:gap-3 py-3 desktop:py-4.5 px-4 desktop:px-6 [@media(max-width:767px)_and_(max-height:500px)_and_(orientation:landscape)]:flex-row [@media(max-width:767px)_and_(max-height:500px)_and_(orientation:landscape)]:justify-between [@media(max-width:767px)_and_(max-height:500px)_and_(orientation:landscape)]:py-1 md:grid md:grid-cols-[1fr_auto_1fr] md:items-center md:min-h-[5vh] md:gap-0 md:py-0">
      ...
      {/* `max-w-full` overrides the menu root's own `max-w-max` so it can wrap now. The bar's middle column is `auto` rather
