...
export function ProjectCard({
  ...
  return (
    ...
    <article className="flex h-full w-full flex-col items-center gap-4 desktop:gap-8 rounded-lg desktop:rounded-2xl border border-border bg-primary/20 p-6 desktop:p-12 [@media(max-height:500px)_and_(orientation:landscape)]:flex-row [@media(max-height:500px)_and_(orientation:landscape)]:p-4">
      <Image
        src={project.src}
        alt={project.alt}
        width={1200}
        ...
        sizes="(min-width: 1024px) 800px, (min-width: 640px) 400px, 76vw"
        priority={priority}
      ...
      <div className="flex w-full flex-1 flex-col items-center gap-4 desktop:gap-8 [@media(max-height:500px)_and_(orientation:landscape)]:self-stretch [@media(max-height:500px)_and_(orientation:landscape)]:justify-center [@media(max-height:500px)_and_(orientation:landscape)]:gap-2">
        <h2 className="text-center text-2xl desktop:text-5xl font-semibold [@media(max-height:500px)_and_(orientation:landscape)]:text-lg">
          {project.name}
        </h2>
        <p className="text-center text-xl desktop:text-[2.5rem] [@media(max-height:500px)_and_(orientation:landscape)]:text-sm">
          {project.what}
        </p>
        {/* <p className="text-center text-foreground">{project.why}</p>
        <p className="text-center text-foreground">{project.hardBit}</p> */}

        <ul className="flex flex-wrap justify-center gap-2 desktop:gap-4 text-sm desktop:text-[1.75rem] text-muted-foreground [@media(max-height:500px)_and_(orientation:landscape)]:text-xs">
          {project.stack.map((t) => (
            <li
              key={t}
        ...
        <div className="mt-auto flex gap-4 desktop:gap-8 pt-2 desktop:pt-4 desktop:text-2xl [@media(max-height:500px)_and_(orientation:landscape)]:mt-0 [@media(max-height:500px)_and_(orientation:landscape)]:pt-0 [@media(max-height:500px)_and_(orientation:landscape)]:text-sm">
          <Link
            href={project.href}
