...
export default async function resultsPage({
  ...
  async function fetchResults1() {
    const resp = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=${process.env.GH_API_KEY_1}&q=${p}&limit=10&offset=0&rating=r&lang=en&bundle=messaging_non_clips`)
    const json = await resp.json()
    return json.data
  }

  async function fetchResults2() { 
    const resp = await fetch(`https://api.klipy.com/api/v1/${process.env.GH_API_KEY_2}/gifs/search?page=1&per_page=24&q=${p}&customer_id=guest&locale=uk&content_filter=off`)
    const json = await resp.json()
